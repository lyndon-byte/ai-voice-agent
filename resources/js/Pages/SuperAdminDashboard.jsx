...
// Resolve a raw note string to a DISABLE_REASONS entry (if it matches), or return null
function resolveReason(note) {
  if (!note) return null;
  return DISABLE_REASONS.find(
    (r) => r.id === note || r.label.toLowerCase() === note.toLowerCase()
  ) ?? null;
}

// ── DisableAgentsModal ──────────────────────────────────────────────────────
// Props:
//   org            – full org object (needs org.id, org.name, org.note)
//   isDisabled     – boolean: current disabled status (org.active === false)
//   onClose        – close handler
//   onConfirm(cmd, reason) – called after successful API response
//                    cmd    : "on" | "off"
//                    reason : reason string sent to backend

function DisableAgentsModal({ org, isDisabled, onClose, onConfirm }) {
  const [selectedReason, setSelectedReason] = useState(
    isDisabled ? (org.note ?? null) : null
  );
  const [loading, setLoading] = useState(false);
  ...
  const handleConfirm = async () => {
    if (!isDisabled && !selectedReason) return;

    const command = isDisabled ? "on" : "off";
    // When re-enabling, we still need to send a reason per backend validation.
    // Use the existing org.note (reason that caused the disable).
    const reason = isDisabled ? (org.note ?? selectedReason) : selectedReason;

    setLoading(true);
    setError(null);

    try {
      await axios.post("/admin/switch-agents", {
        org_id: org.id,
        reason,
        command,
      });
      ...
      <div className="relative bg-[#0f1117] border border-white/10 rounded-2xl p-8 max-w-md w-full shadow-2xl shadow-black/60">
        <div
          className={`absolute top-0 left-8 right-8 h-px bg-gradient-to-r from-transparent ${
            isDisabled ? "via-emerald-500" : "via-rose-500"
          } to-transparent`}
        />
        ...
        {done ? (
          <div className="text-center py-4">
            ...
            >
              <svg
                className={`w-8 h-8 ${isDisabled ? "text-emerald-400" : "text-rose-400"}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-white font-semibold text-lg">
              {isDisabled ? "Agents Re-enabled" : "Agents Disabled"}
            </p>
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              >
                <svg
                  className={`w-5 h-5 ${isDisabled ? "text-emerald-400" : "text-rose-400"}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div>
                <h3 className="text-white font-semibold">
                  {isDisabled ? "Re-enable Agents" : "Disable All Agents"}
                </h3>
            ...
            {isDisabled ? (
              <>
                <div className="bg-white/5 rounded-xl p-4 mb-5 border border-white/5">
                  <p className="text-zinc-400 text-xs mb-2">Currently disabled for</p>
                  {/* Show matched reason badge or raw org.note */}
                  {(() => {
                    const matched = resolveReason(org.super_admin_note);
                    return matched ? (
                      <span
                    ...
                    ) : org.super_admin_note ? (
                      <p className="text-zinc-300 text-sm leading-relaxed">{org.super_admin_note}</p>
                    ) : (
                      <span className="text-zinc-500 text-xs italic">No reason specified</span>
                    );
                  })()}
                </div>
                <p className="text-zinc-400 text-sm mb-6 leading-relaxed">
                  This will restore all agent access for{" "}
                  <span className="text-white font-medium capitalize">{org.name}</span>.{" "}
                  Make sure the issue has been resolved before proceeding.
                </p>
              </>
            ) : (
              <>
                <p className="text-zinc-400 text-sm mb-4 leading-relaxed">
                  Select the reason for disabling all agents in{" "}
                  <span className="text-white font-medium capitalize">{org.name}</span>.{" "}
            ...
            {/* API error */}
            {error && (
              <div className="mb-4 flex items-center gap-2 text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={handleConfirm}
                disabled={loading || (!isDisabled && !selectedReason)}
...
// ── OrgCard ─────────────────────────────────────────────────────────────────
// Drives disabled UI from org.active (false = disabled) + org.note (reason).
// Local shadow state mirrors the server value so the UI updates instantly
// after a successful POST without needing a full page reload.

function OrgCard({ org }) {
  const [expanded, setExpanded] = useState(true);
  const [disableModal, setDisableModal] = useState(false);

  // Shadow state: { active: bool, note: string|null }
  // Initialized from the server-provided org props.
  const [agentState, setAgentState] = useState({
    active: org.active,
    note: org.super_admin_note ?? null,
  });

  const isDisabled = !agentState.active;

  // Called by DisableAgentsModal after a successful API response.
  // cmd: "on" | "off", reason: string
  const handleAgentToggle = (cmd, reason) => {
    setAgentState({
      active: cmd === "on",
      note: cmd === "off" ? reason : null,
    });
  };

  // Resolve note to a known reason for badge display
  const matchedReason = resolveReason(agentState.note);

  return (
    <>
      <div
        className={`bg-[#0f1117] border rounded-2xl overflow-hidden transition-all ${
          isDisabled
      ...
      >
        ...
        {isDisabled && (
          <div className="flex items-center justify-between gap-3 px-5 py-2.5 bg-rose-500/[0.08] border-b border-rose-500/20">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-rose-400 text-xs font-medium whitespace-nowrap">
                All agents disabled
              </span>
              {agentState.note && (
                <>
                  <span className="text-rose-500/60 text-xs">·</span>
                  {/* Show matched label or raw note */}
                  <span className="text-rose-400/70 text-xs truncate">
                    {matchedReason ? matchedReason.label : agentState.note}
                  </span>
                </>
              )}
            </div>
            <button
              ...
              className="text-[11px] text-emerald-400 hover:text-emerald-300 font-medium underline underline-offset-2 transition-colors whitespace-nowrap"
            >
              Re-enable
            </button>
          </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            >
              <svg
                className={`w-5 h-5 ${isDisabled ? "text-rose-400/60" : "text-violet-400"}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className={`font-semibold capitalize ${isDisabled ? "text-zinc-400" : "text-white"}`}>
                  {org.name}
                </h3>
          ...
          <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              {isDisabled ? "Re-enable Agents" : "Disable Agents"}
            </button>
      ...
      {disableModal && (
        <DisableAgentsModal
          org={{ ...org, note: agentState.note }}
          isDisabled={isDisabled}
          onClose={() => setDisableModal(false)}
          onConfirm={handleAgentToggle}
        />
      )}
    </>
  );
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
            {[
              ...
              },
              {
                label: "Avg Users / Org",
                value: organizations.length
                  ? (totalUsers / organizations.length).toFixed(1)
                  : 0,
                icon: (
              ...
              >
                <div
                  className={`w-9 h-9 rounded-lg bg-${stat.color}-500/10 border border-${stat.color}-500/20 flex items-center justify-center flex-shrink-0`}
                >
                  <svg
                    className={`w-4.5 h-4.5 text-${stat.color}-400`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
